...
class HomeTab:
    def __init__(self, parent, chat_ctrl: ChatController):
        ...
        session_status_label = ctk.CTkLabel(
            _inner_frame, textvariable=self.session_status_var, height=20, width=150
        )
        session_status_label.grid(row=0, column=1, sticky="w", padx=(10, 0))

        self.party_size_var = ctk.IntVar(
            value=self.config.getint(section="DND", option="party_size", fallback=4)
        )
        self.party_label_var = ctk.StringVar(
            value=f"Party Size - {self.party_size_var.get()}"
        )
        ...
        self.start_session = ctk.CTkButton(
            _inner_frame, text="Start Session", command=self._start_session
        )
        ...
        self.end_button = ctk.CTkButton(
            _inner_frame, text="End Session", command=self._end_session
        )
        ...
        self.queue_label_var = ctk.StringVar(
            value=f"{len(self.chat_ctrl.session_mgr.session.queue)} in Queue"
        )
        queue_label = ctk.CTkLabel(
            _inner_frame_queue, textvariable=self.queue_label_var, height=20, width=306
        )
    ...
    def _add_chat_msg(self, member, message):
        ...
        self._chat_frame.after(
            100, lambda: self._chat_frame._parent_canvas.yview_moveto(1.0)
        )
    ...
    def _fill_party_frame(self):
        ...
        for index, member in enumerate(
            sorted(self.chat_ctrl.session_mgr.session.party)
        ):
            row = index // columns
            col = index % columns
            ...
            member_card.grid(
                row=row, column=col, padx=(35, 10), pady=(12, 12), sticky="w"
            )
    ...
    def _open_session(self):
        ...
        self.session_status_var.set(
            value=self.chat_ctrl.session_mgr.session.state.name.capitalize()
        )
    ...
    def _update_party_limit(self, value):
        if int(value) != self.config.getint(
            section="DND", option="party_size", fallback=-1
        ):
