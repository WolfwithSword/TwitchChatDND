...
class UsersTab:
    def __init__(self, parent, chat_ctrl: ChatController):
        ...
        self.prev_button = ctk.CTkButton(
            self.parent, text="Previous", command=self.previous_page
        )
        self.prev_button.pack(side=ctk.LEFT, padx=10, pady=10)

        self.next_button = ctk.CTkButton(
            self.parent, text="Next", command=self.next_page
        )
        self.next_button.pack(side=ctk.LEFT, padx=10, pady=10)

        self.refresh_button = ctk.CTkButton(
            self.parent, text="Refresh", command=self.schedule_load_members
        )
    ...
    async def load_members(self):
        ...
        async with self.load_members_lock:
            ...
            try:
                await asyncio.sleep(0.2)
                members = await fetch_paginated_members(
                    self.page, self.per_page, name_filter=self.name_filter
                )
                ...
                for member in members:
                    member_card = MemberCard(
                        self.members_list_frame, member, self.chat_ctrl.config
                    )
    ...
    def update_pagination_buttons(self):
        ...
        self.next_button.configure(
            state=(
                "normal"
                if len(self.members_list_frame.winfo_children()) == self.per_page
                else "disabled"
            )
        )
