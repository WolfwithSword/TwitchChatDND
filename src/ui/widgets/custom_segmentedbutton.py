...
from customtkinter import ThemeManager, CTkFont, CTkButton, CTkFrame
...
def check_kwargs_empty(kwargs_dict, raise_error=False) -> bool:
    ...
    if len(kwargs_dict) > 0:
        if raise_error:
            raise ValueError(
                f"{list(kwargs_dict.keys())} are not supported arguments. Look at the documentation for supported arguments."
            )
        else:
            return True
    else:
...
class CTkSegmentedButton(CTkFrame):
    ...
    def __init__(
        ...
        self._sb_fg_color = (
            ThemeManager.theme["CTkSegmentedButton"]["fg_color"]
            if fg_color is None
            else self._check_color_type(fg_color)
        )

        self._sb_selected_color = (
            ThemeManager.theme["CTkSegmentedButton"]["selected_color"]
            if selected_color is None
            else self._check_color_type(selected_color)
        )
        self._sb_selected_hover_color = (
            ThemeManager.theme["CTkSegmentedButton"]["selected_hover_color"]
            if selected_hover_color is None
            else self._check_color_type(selected_hover_color)
        )

        self._sb_unselected_color = (
            ThemeManager.theme["CTkSegmentedButton"]["unselected_color"]
            if unselected_color is None
            else self._check_color_type(unselected_color)
        )
        self._sb_unselected_hover_color = (
            ThemeManager.theme["CTkSegmentedButton"]["unselected_hover_color"]
            if unselected_hover_color is None
            else self._check_color_type(unselected_hover_color)
        )

        self._sb_text_color = (
            ThemeManager.theme["CTkSegmentedButton"]["text_color"]
            if text_color is None
            else self._check_color_type(text_color)
        )
        self._sb_text_color_disabled = (
            ThemeManager.theme["CTkSegmentedButton"]["text_color_disabled"]
            if text_color_disabled is None
            else self._check_color_type(text_color_disabled)
        )

        self._sb_corner_radius = (
            ThemeManager.theme["CTkSegmentedButton"]["corner_radius"]
            if corner_radius is None
            else corner_radius
        )
        self._sb_border_width = (
            ThemeManager.theme["CTkSegmentedButton"]["border_width"]
            if border_width is None
            else border_width
        )

        self._background_corner_colors = (
            background_corner_colors  # rendering options for DrawEngine
        )
        ...
        self._buttons_dict: Dict[str, CTkButton] = (
            {}
        )  # mapped from value to button object
        if values is None:
            self._value_list: List[str] = ["CTkSegmentedButton"]
        else:
            self._value_list: List[str] = (
                values  # Values ordered like buttons rendered on widget
            )
    ...
    def _configure_button_corners_for_index(self, index: int):
        if index == 0 and len(self._value_list) == 1:
            ...
            if self._background_corner_colors is None:
                ...
                self._buttons_dict[self._value_list[index]].configure(
                    background_corner_colors=(
                        self._background_corner_colors[0],
                        self._sb_fg_color,
                        self._sb_fg_color,
            ...
            if self._background_corner_colors is None:
                ...
                self._buttons_dict[self._value_list[index]].configure(
                    background_corner_colors=(
                        self._sb_fg_color,
                        self._background_corner_colors[1],
                        self._background_corner_colors[2],
                        self._sb_fg_color,
                    )
                )
    ...
    def _unselect_button_by_value(self, value: str):
        if value in self._buttons_dict:
            self._buttons_dict[value].configure(
    ...
    def _select_button_by_value(self, value: str):
        ...
        self._buttons_dict[value].configure(
            fg_color=self._sb_selected_color, hover_color=self._sb_selected_hover_color
        )
    ...
    def configure(self, **kwargs):
        ...
        if "unselected_hover_color" in kwargs:
            self._sb_unselected_hover_color = self._check_color_type(
                kwargs.pop("unselected_hover_color")
            )
            for value, button in self._buttons_dict.items():
                if value != self._current_value:
    ...
    def insert(self, index: int, value: str):
        if value not in self._buttons_dict:
            ...
            raise ValueError(
                f"CTkSegmentedButton can not insert value '{value}', already part of the values"
            )
    ...
    def move(self, new_index: int, value: str):
        if 0 <= new_index < len(self._value_list):
            ...
            raise ValueError(
                f"CTkSegmentedButton new_index {new_index} not in range of value list with len {len(self._value_list)}"
            )
    ...
    def delete(self, value: str):
        if value in self._buttons_dict:
            ...
            if (
                index_to_remove == len(self._buttons_dict)
                and len(self._buttons_dict) > 0
            ):
